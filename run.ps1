...
$ExePath = Join-Path $RepoRoot "src-tauri\target\$TargetDir\MessagesEncrypter.exe"

# 运行中的实例会锁住 exe 导致链接器拒绝写入，必须先退出
Get-Process MessagesEncrypter -ErrorAction SilentlyContinue | Stop-Process -Force
