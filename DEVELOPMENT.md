...
- 修正设置页仓库链接：指向本仓库 github.com/BlazeSnow/MessagesEncrypter.Rust（此前误用旧 WinUI 仓库地址）；双语描述同步。references/ 中保留的旧仓库 URL 属历史事实，不改。
...
- 首页使用说明加全局序号：接收方 1~3（生成私钥/分享公钥/解密密文包）、发送方 4~5（导入公钥/加密消息），步骤前用主题色圆形序号标替代装饰图标；已截图验证。

### 2026-09-26（十一）

- 加解密两页去掉最外层大卡片：表单元素（密钥选择、输入框、按钮组）直接平铺，与密钥管理页布局风格统一；页内不再重复大标题（导航已标明位置，沿用原版「页面内不放重复大标题」约定）。

### 2026-09-26（十）

- 加解密页密钥下拉：指纹部分改用 text-muted-foreground 浅色（与密钥卡片一致），别名保持正常色；选中所带出的触发器文本同步生效。

### 2026-09-26（九）

- 加解密页密钥选择记忆加固：功能本就存在（选择写 settings 表、进页恢复，CDP 实测两页恢复正常），但存在竞态——进页后恢复请求在途时用户手动选择，恢复完成会覆盖手选值，表现为「每次都要重选」。修复：用户手动选择后恢复流程不得覆盖（ref 标记）；「自动选中第一把」的回退值现在也写入记忆。已用 WebView2 远程调试（CDP）实测页面切换与重挂载后两页均正确恢复。

### 2026-09-26（八）

- 单实例唤醒补强：插件早已有（脚手架期接入，双启动仅 1 进程），但原回调只 set_focus——tao 的 set_focus 在窗口最小化时直接跳过，用户感知为「二次启动没反应」。回调改为 unminimize + show + set_focus（用户实测最小化下二次启动可正常还原）。

### 2026-09-26（七）

- 加解密按钮加载动画：加密 / 解密 / 解锁私钥三个按钮在 busy 时图标切换为旋转 Loader2（解密需 600k 次 PBKDF2 解锁私钥，约 1 秒），与生成密钥的进度反馈风格一致。
...
- 弹窗通用修正：导入弹窗 DialogContent 加 `max-h-[calc(100dvh-2rem)] overflow-y-auto`（矮窗口下可滚动），宽度改用 `sm:max-w-xl` 正确覆盖基类的 `sm:max-w-sm`（此前被压成 384px）。
